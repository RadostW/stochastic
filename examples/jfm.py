...
if False:
    trajectories = solver.solve_many(
       problem, n_trajectories=1, chunk_size=chunk_size, 
    chunks_per_randomization=1, seed = 2
    )
    single = np.array(trajectories["solution_values"][0])
    np.savetxt("single_trajectory.csv",single, delimiter=",")

trajectories = solver.solve_many(
    problem, n_trajectories=2 ** 8, chunk_size=chunk_size, chunks_per_randomization=1, seed = 0
)
...
np.savetxt("big_bead_msd.csv",np.array(big_bead_msd), delimiter=",")
...
np.savetxt("centre_msd.csv",np.array(centre_msd), delimiter=",")
